...
class Node:
    ...
    def create_publisher(self, msg_type: type, topic_name: str, qos_profile: QoSProfile = None):
        """
        ...
        if not self._is_initialized:
            raise NodeNotInitializedError("Node must be initialized before creating publishers")
    ...
    def create_subscription(self, msg_type: type, topic_name: str, callback: Callable, 
        ...
        """
        if not self._is_initialized:
            raise NodeNotInitializedError("Node must be initialized before creating subscriptions")
    ...
    def create_service(self, srv_type: type, service_name: str, callback: Callable, 
        ...
        """
        if not self._is_initialized:
            raise NodeNotInitializedError("Node must be initialized before creating services")
    ...
    def create_client(self, srv_type: type, service_name: str, qos_profile: QoSProfile = None):
        """
        ...
        """
        if not self._is_initialized:
            raise NodeNotInitializedError("Node must be initialized before creating clients")
    ...
    def create_timer(self, timer_period_sec: float, callback: Callable):
        ...
        """
        if not self._is_initialized:
            raise NodeNotInitializedError("Node must be initialized before creating timers")
    ...
    def declare_parameter(self, name: str, default_value: Any = None, 
        ...
        if name in self._parameters:
            raise ParameterAlreadyDeclaredError(f"Parameter '{name}' is already declared")
    ...
    def set_parameter(self, name: str, value: Any) -> bool:
        ...
        if expected_type != actual_type:
            raise TypeError(f"Parameter '{name}' expects type '{expected_type}' but got '{actual_type}'")
